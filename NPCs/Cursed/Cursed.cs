...
namespace BrandonBox.NPCs.Cursed
{

	public class CursedCollection : ModSystem
	{
		public static int counter = 9999;
		public override void PreUpdateTime() {
			if ((Main.time + 1800) % 3600 == 0)
				counter+=1;
			if (counter >= 24 * 3 && NPC.AnyNPCs(ModContent.NPCType<Cursed>()))
			{
				int i = 0;
				bool hasany = false;
	...
	public class Cursed : ModNPC
	{
		...
		public override bool CanTownNPCSpawn(int numTownNPCs) { // Requirements for the town NPC to spawn.
			...
			if (!ModContent.GetInstance<Systems.NPCsConfigs>().Cursed) return false;
			if (CursedCollection.counter < 24 * 3) return false;
